...
def plugin_tags_to_admin_html(text):
    """ Convert plugin object 'tags' into the form used to represent them in the admin text editor. """
    ...
    def _tag_to_admin(m):
        ...
        try:
            ...
            # Object must have been deleted.  It cannot be rendered to end user, or edited, so just remove it from the HTML altogether
            return u''
...
def plugin_admin_html_to_tags(text):
    """ Convert the HTML used in admin editor to represent plugin objects into the 'tag' form used in the database """
